      ...
      void main(void) {
          ...
          for (int i = 0;i < 180; i++)
          {
              if (blob(o + d * t) < 20.)
              {
                  ...
                  vec3 n = vec3(.0);
                  n.x = blob(o + d * t) - blob(vec3(o + d * t + e.xyy)) - (iZoomOffset * 5.0);
                  n.y = blob(o + d * t) - blob(vec3(o + d * t + e.yxy)) - (iZoomOffset * 5.0);
          ...
          // Base Color
          gl_FragColor += vec4(.16, 0.05, .38, 1.) * (0.8 + (iZoomOffset * 0.3));

          // Specular
          gl_FragColor += c * (SPECULAR_STRENGTH + (iZoomOffset * 0.1)) * vec4(.40, 0.6, 0.9, 1);

          // Brightness
          gl_FragColor *= (t * (.05 + (iZoomOffset * 0.01)));
